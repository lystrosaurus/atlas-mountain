...
public class LoggingBinlogHandler implements BinlogEventHandler {
  ...
  public void onInsert(InsertEvent event) {
    log.info(
        "CDC INSERT {}.{} rows={}", event.getDatabase(), event.getTable(), event.getRows().size());
  }

  @Override
  public void onUpdate(UpdateEvent event) {
    log.info(
        "CDC UPDATE {}.{} rows={}", event.getDatabase(), event.getTable(), event.getRows().size());
  }

  @Override
  public void onDelete(DeleteEvent event) {
    log.info(
        "CDC DELETE {}.{} rows={}", event.getDatabase(), event.getTable(), event.getRows().size());
  }
